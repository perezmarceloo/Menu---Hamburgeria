...
cardapio = {
    "Refri": [':cup_with_straw: Refri de Maçã', ':cup_with_straw: Refri de Cola', ':cup_with_straw: Refri de Laranja', ':cup_with_straw: Refri de Uva', ':cup_with_straw: Refri de Guaraná', ':cup_with_straw: Refri de Limão'],
    ...
    "Brinquedo": ['🧸 Brinquedo do Homem-Morcego', '🧸 Brinquedo do Homem-Aracnídeo', '🧸 Brinquedo da Mulher-Maravilhosa', '🧸 Brinquedo do Rob-Esponja', '🧸 Brinquedo do Perna-Curta']
}
...
async def on_message(message):
    ...
    if user_id in pedidos_em_andamento and not message.content.startswith("!"):
        ...
        if etapa == "refri":
            ...
            estado["etapa"] = "brinquedo"
            
            opcoes_brinquedos = ", ".join(cardapio["Brinquedo"])
            await message.channel.send(
                f"Anotado! **4. Qual brinquedo você deseja?**\n*Opções:* {opcoes_brinquedos}"
            )

        elif etapa == "brinquedo":
            estado["itens"]["Brinquedo"] = texto_usuario
            ...
            resumo = (
                ...
                f"🧸 **Brinquedo:** {estado['itens']['Brinquedo']}\n\n"
...
# Lembre-se de redefinir o token no Portal de Desenvolvedores do Discord!
bot.run("TOKEN AQUI")
